...
public class FilterContactTypeViewModel extends BaseBottomSheetViewModel {
    ...
    private void updateFilterContactTypes() {
        String[] filterContactTypes = ContactType.getContactTypes();// создайте массив строк. Для инициализации используйте метод ContactType.getContactTypes(),
        // который должен возвращать список всех доступных источников контактов

        ArrayList<FilterContactTypeUi> filterContactTypeUisList = new ArrayList<>();// создайте список типа FilterContactTypeUi и заполните его с помощью цикла forEach
        filterContactTypeUisList.add(createAllSelectedItem(filterContactTypes));
        for (String filterContactType : filterContactTypes) {
            filterContactTypeUisList.add(createFilterContactType(filterContactType));
        }// forEach должен бежать по строковому массиву, который вы создали ранее

        filterContactTypesLiveDate.setValue(filterContactTypeUisList); // вызовите меотод setValue() у переменной filterContactTypesLiveDate и передайте в качестве аргументы ваш список типа FilterContactTypeUi
    }
